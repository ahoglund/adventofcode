...
data.each_with_index do |row, r|
    ...
    row.each_with_index do |v, c|
        if ("0".."9").include?(v)
            current_number << v
            if data[r - 1] && !ignored_chars.include?(data[r - 1][c - 1])
                touches_symbol = true
                if data[r - 1][c - 1] == "*"
                  asterisk_coords = [r - 1, c - 1]
                end
            elsif data[r - 1] && !ignored_chars.include?(data[r - 1][c])
                touches_symbol = true
                if data[r - 1][c] == "*"
                  asterisk_coords = [r - 1, c]
                end
            elsif data[r - 1] && !ignored_chars.include?(data[r - 1][c + 1])
                touches_symbol = true
                if data[r - 1][c + 1] == "*"
                  asterisk_coords = [r - 1, c + 1]
                end
            elsif data[r] && !ignored_chars.include?(data[r][c - 1])
                touches_symbol = true
                if data[r][c - 1] == "*"
                  asterisk_coords = [r, c - 1]
                end
            elsif data[r] && !ignored_chars.include?(data[r][c + 1])
                touches_symbol = true
                if data[r][c + 1] == "*"
            ...
            if c == (row.length - 1)
                numbers << current_number if touches_symbol
                unless asterisk_coords.empty?
                    gear_coords[asterisk_coords.join] ||= []
                    gear_coords[asterisk_coords.join] << current_number.join
                    asterisk_coords = []
                end
                current_number = []
                touches_symbol = false
...
# part 1
puts numbers.map { |n| n.join.to_i }.sum

# part 2
puts gear_coords.values.reject { |n| n.size == 1 }.map { |nums| nums.map(&:to_i).reduce(:*) }.sum
