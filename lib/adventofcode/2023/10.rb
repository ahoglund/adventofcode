...
def detect_connecting_pipes(x, y, data)
    connecting_pipes = []

    case data[y][x]
    when "S"
        if data[y-1] && (data[y-1][x] == "|" || data[y-1][x] == "F" || data[y-1][x] == "7" || data[y-1][x] == "S")
            connecting_pipes << [x, y-1]
        end

        if data[y+1] && (data[y+1][x] == "|" || data[y+1][x] == "L" || data[y+1][x] == "J" || data[y+1][x] == "S")
            connecting_pipes << [x, y+1]
        end

        if data[y][x-1] && (data[y][x-1] == "-" || data[y][x-1] == "F" || data[y][x-1] == "L" || data[y][x-1] == "S")
            connecting_pipes << [x-1, y]
        end

        if data[y][x+1] && (data[y][x+1] == "-" || data[y][x+1] == "J" || data[y][x+1] == "7" || data[y][x+1] == "S")
            connecting_pipes << [x+1, y]
        end
    when "F"
        # | or J or L or S
    ...
    when "L"
        # | or 7 or F or S
        if data[y-1] && (data[y-1][x] == "|" || data[y-1][x] == "7" || data[y-1][x] == "F" || data[y-1][x] == "S")
            connecting_pipes << [x, y-1]
        end
    ...
    when "J"
        ...
        # - or L or F or S
        if data[y][x-1] && (data[y][x-1] == "-" || data[y][x-1] == "L" || data[y][x-1] == "F" || data[y][x-1] == "S")
            connecting_pipes << [x-1, y]
        end
    when "7"
        # | or J or L or S
        if data[y+1] && (data[y+1][x] == "|" || data[y+1][x] == "J" || data[y+1][x] == "L" || data[y+1][x] == "S")
            connecting_pipes << [x, y+1]
        end
...
puts start.inspect
prev_pipe = start
next_pipe = detect_connecting_pipes(*start, data).first
visited = Set.new
visited.add(start)
visited.add(next_pipe)
while true do
    current_pipe = next_pipe
    next_pipe = detect_connecting_pipes(*current_pipe, data).reject { |pipe| pipe == prev_pipe }.first
    # puts "CURRENT:#{current_pipe} NEXT: #{next_pipe} DET: #{detect_connecting_pipes(*current_pipe, data)}"
    # puts "DATA: #{data[current_pipe[1]][current_pipe[0]]}"
    prev_pipe = current_pipe
    visited.add(next_pipe)

    break if next_pipe == start
end

puts visited.size / 2
# enclosed = 0
# data.each_with_index do |row, y|
#   data[y].each_with_index do |cell, x|
#         # 1 cannot be on a outside edge
#         # 2 it has to be a .
#         # 3 it has to be surrounded by either a .
#         # L J 7 F S - |
#
#     # if cell == "." && visited.include?([x + 1, y]) && visited.include?([x - 1, y]) && visited.include?([x, y + 1]) && visited.include?([x, y - 1])
#     #         enclosed +=1
#     # end
#   end
# end
